# -*- coding: utf-8 -*-
from odoo import api, fields, models, _
from odoo.exceptions import ValidationError

class HrEmployee(models.Model):
    _inherit = 'hr.employee'

    # ====== Fields from HEAD ======
    employee_code = fields.Char(
        string='Employee Code',
        required=True,
        copy=False,
        readonly=False,
    )
    employee_status = fields.Selection(
        selection=[
            ('active', 'Active'),
            ('inactive', 'Inactive'),
            ('resigned', 'Resigned'),
        ],
        string='Employee Status',
        required=True,
        default='active',
    )
    join_date = fields.Date(
        string='Join Date',
        required=True,
    )
    nik = fields.Char(
        string='NIK',
        size=16,
        required=True,
        index=True,
        copy=False,
    )
    npwp_number = fields.Char(
        string='NPWP Number',
        required=True,
    )
    employee_type = fields.Selection(
        selection=[
            ('tetap', 'Tetap'),
            ('freelance', 'Freelance'),
        ],
        string='Employee Type',
        required=True,
        default='tetap',
    )
    bank_name = fields.Char(string='Bank Name')
    bank_account_number = fields.Char(string='Bank Account Number')

    # ====== Fields from commit (baru) ======
    x_fingerprint_id = fields.Char(
        string='Fingerprint ID',
        help='Unique PIN identifier registered on the fingerprint machine.',
        copy=False,
        index=True,
    )
    x_whatsapp_number = fields.Char(
        string='WhatsApp Number',
        help='Employee WhatsApp number for notifications (e.g., 628123456789).',
    )
    x_tax_status = fields.Selection(
        selection=[
            ('TK/0', 'TK/0 - Tidak Kawin, Tanpa Tanggungan'),
            ('TK/1', 'TK/1 - Tidak Kawin, 1 Tanggungan'),
            ('TK/2', 'TK/2 - Tidak Kawin, 2 Tanggungan'),
            ('TK/3', 'TK/3 - Tidak Kawin, 3 Tanggungan'),
            ('K/0', 'K/0 - Kawin, Tanpa Tanggungan'),
            ('K/1', 'K/1 - Kawin, 1 Tanggungan'),
            ('K/2', 'K/2 - Kawin, 2 Tanggungan'),
            ('K/3', 'K/3 - Kawin, 3 Tanggungan'),
            ('K/I/0', 'K/I/0 - Kawin, Penghasilan Istri Digabung, 0 Tanggungan'),
            ('K/I/1', 'K/I/1 - Kawin, Penghasilan Istri Digabung, 1 Tanggungan'),
            ('K/I/2', 'K/I/2 - Kawin, Penghasilan Istri Digabung, 2 Tanggungan'),
            ('K/I/3', 'K/I/3 - Kawin, Penghasilan Istri Digabung, 3 Tanggungan'),
        ],
        string='Tax Status (PTKP)',
        default='TK/0',
        help='Indonesian tax status used for PPh 21 TER category mapping.',
    )

    # ====== SQL Constraints ======
    _sql_constraints = [
        ('uniq_employee_code', 'UNIQUE(employee_code)', 'Employee code must be unique.'),
        ('uniq_nik', 'UNIQUE(nik)', 'NIK must be unique.'),
        ('unique_fingerprint_id', 'UNIQUE(x_fingerprint_id)', 'Fingerprint ID must be unique per employee.'),
    ]

    # ====== Override create ======
    @api.model
    def create(self, vals):
        # Auto-generate employee_code if not provided
        if not vals.get('employee_code'):
            vals['employee_code'] = self.env['ir.sequence'].next_by_code('hris.employee.code')

        # Validate and sync NIK with identification_id
        if vals.get('nik'):
            if len(vals['nik']) != 16 or not vals['nik'].isdigit():
                raise ValidationError(_('NIK must be 16 digits.'))
            vals['identification_id'] = vals.get('identification_id') or vals['nik']

        # If active=False then set employee_status = 'resigned'
        if vals.get('active') is False:
            vals['employee_status'] = vals.get('employee_status') or 'resigned'

        return super().create(vals)

    # ====== Override write ======
    def write(self, vals):
        # Sync NIK to identification_id
        if vals.get('nik'):
            if len(vals['nik']) != 16 or not vals['nik'].isdigit():
                raise ValidationError(_('NIK must be 16 digits.'))
            vals['identification_id'] = vals.get('identification_id') or vals['nik']

        # Sync between employee_status and active (employee_status takes priority)
        if 'employee_status' in vals:
            if vals['employee_status'] == 'resigned':
                vals['active'] = False
            elif vals['employee_status'] in ('active', 'inactive'):
                if vals.get('active') is None:
                    vals['active'] = True
        else:
            # Only active is changed, adjust employee_status accordingly
            if vals.get('active') is False:
                vals['employee_status'] = vals.get('employee_status') or 'resigned'
            elif vals.get('active') is True and not vals.get('employee_status'):
                vals['employee_status'] = 'active'

        return super().write(vals)

    # ====== Constraint for NIK format ======
    @api.constrains('nik')
    def _check_nik_format(self):
        for rec in self:
            if rec.nik and (len(rec.nik) != 16 or not rec.nik.isdigit()):
                raise ValidationError(_('NIK must be 16 digits.'))

    # ====== Helper to sync NIK from identification_id (optional) ======
    @api.model
    def _sync_nik_from_identification_id(self):
        """Call this method manually or via cron to populate nik from identification_id if empty."""
        for rec in self.search([('nik', '=', False), ('identification_id', '!=', False)]):
            rec.nik = rec.identification_id

    # ====== Related helper fields for API / exports ======
    full_name = fields.Char(
        string='Full Name',
        related='name',
        store=False,
        readonly=True,
    )
    email = fields.Char(
        string='Email',
        related='work_email',
        store=False,
        readonly=True,
    )
    phone_number = fields.Char(
        string='Phone Number',
        related='mobile_phone',
        store=False,
        readonly=True,
    )

    # ====== Method for portal export ======
    def _prepare_portal_export_values(self):
        self.ensure_one()
        return {
            'employee_id': self.id,
            'employee_code': self.employee_code,
            'nik': self.nik,
            'full_name': self.name,
            'email': self.work_email,
            'phone_number': self.mobile_phone,
            'employee_type': self.employee_type,
            'department': self.department_id.name if self.department_id else None,
            'job_position': self.job_id.name if self.job_id else None,
            'join_date': self.join_date,
            'status': self.employee_status,
        }